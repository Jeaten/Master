...
class dA2(dA):#########relu线性函数部分###########
    ...
    def __init__(
        ...
        if W is None:
            ...
            initial_W = numpy.asarray(1 / numpy.sqrt(self.n_visible) *numpy.float32(numpy.random.randn(n_visible, n_hidden)))
        else:
            initial_W = W
...
#        cost = L
        ...
        # update weights
        # ############更新权重##################
        updates.append((self.delta_W,mu * self.delta_W - learning_rate * gparams_W))
...
def load_data(dataset):
    ...
    # train_y = train_y[idx]
    print('load_data')
    print('tran_x=',len(train_x))
    print( 'tran_y=', len( train_y ) )
...
def load_data_shared(dataset, batch_size):
    ...
    with gzip.open(dataset, 'rb') as f:
        train_x, train_y = cPickle.load(f)
    print('original:',len(train_x),len(train_y))
    N = train_x.shape[0] - train_x.shape[0] % batch_size
    train_x = train_x[0: N]
    train_y = train_y[0: N]
    print('load_data_shared')
    print('train_x',len(train_x))
    print( 'train_y', len( train_y ) )
    #     print( 'create dir failed!' )
    # numpy.save( Jeaten_Path+'original_x.npy', train_x )
    # numpy.save( Jeaten_Path+'original_y.npy', train_y )
    # shuffling
    # numpy.random.seed(0)
    # idx = numpy.random.permutation(N)
...
def shared_dataset(data_xy, borrow=True):
        ...
        # print('shared_x',len(shared_x))
        # print('shared_y',len(shared_y))
...
#    inputargs, _ = arguments() # get the input arguments
    ...
    train_set_x, train_set_y = datasets[0]
    numpy.save( Jeaten_Path+'train_set_x.npy', train_set_x )
    # numpy.save( Jeaten_Path+'train_set_y.npy', train_set_y )
    print( 'loaded dataset share' )
    print(  train_set_x.get_value )
    print( train_set_y.get_value )
    print('test_Sdc')
    # print('train_set_x',len(train_set_x))
    # print('train_set_y',len(train_set_y))
...
#    print 'The random seed is: %d' % seed
    ...
    if pretraining_epochs == 0 or Init != '':
        ...
        pretrain_lr_shared = theano.shared(numpy.asarray(pretrain_lr_base,dtype=floatX),borrow=True)
    ...
    def init_cluster(data):
        numpy.save( Jeaten_Path+'cluster_data.npy', data )
        print('init_cluster')
        print(len(data))
        print(len(data[0]))
        if clusterModel == 'KM':
            ...
            km.fit(data)
            idx = km.labels_
            # numpy.save( Jeaten_Path+'kmeans_cluster_idx.npy', idx )
            centers = km.cluster_centers_
            # numpy.save( Jeaten_Path+'kmeans_cluster_centers.npy', centers )
        elif clusterModel == 'EMC':
...
#    hidden_array  = normalize(hidden_array, norm='l2', axis=1)
    print('hidden_array')
    print( len(hidden_array))
    print(len(hidden_array[0]))
    print('label_true')
    print(label_true)
...
#         evaluate the clustering performance every 5 epoches
    ...
    idx, centers = init_cluster(hidden_array)
    numpy.save( Jeaten_Path+'fin_idx.npy', idx )
    numpy.save( Jeaten_Path+'fin_centers.npy', centers )
#    err = numpy.mean(d)
