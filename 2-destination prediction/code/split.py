...
def cluster():
    ...
    train_seg_all = dat.load_pkl( path + 'train_seg' )
    train_seg={}
    for k in train_seg_all:
        train_seg[k]=train_seg_all[k]
        if k>Num_tra_clu:break
    width=[40,80,120,160,200]#
    pool = multiprocessing.Pool( width.__len__() )
    for i in range( width.__len__() ):
        arg=(train_seg,width[i],str(Num_tra_clu))
        pool.apply_async(clu.cluster,args=arg,callback=save_model )
    pool.close()
    pool.join()
    ### original
...
def completion(tra_ori,center,prefix):
    ...
    for per in percentage:
        tra_per={}
        for k in tra_ori:
            len=int(tra_ori[k].__len__()*per+0.5)
            print(tra_ori[k].__len__(),tra_ori[k].__len__()*per,len)
    ...
    for com_seg in tra_com_seg:
        tra_com_c.append(classify_multi(com_seg,50,center))
...
def train(train_class,center,lab_true,layer=1,com=''):
    ...
    act_ini='tanh'
    loss_ini='mse'
    print('neuron_ini',neuron_ini)
    print( 'training '+str(layer)+'-layer model with completion of '+str(Com*10)+' ...' )
...
if __name__ == '__main__':
    ...
    # path='./file/center_an45/'
    # # width = [40, 60, 80, 100, 120, 140, 160, 180, 200, 220, 240, 300]
    # width=[40,80,120,160,200]
    # tra_seg_all = dat.load_pkl( './file/train_seg' )
    # tra_seg = {}
    # for k in tra_seg_all:
    #     tra_seg[k] = tra_seg_all[k]
    #     if k > 3000: break  # we only choose 30000 trajectories to simulate
    # for w in width:
    #     center=dat.load_pkl(path+str(w))
    # ### about test set
    # ### test_seg=dat.load_pkl(path+'test_seg')
    # ### test_class=classify_multi(test_seg,30,center)
    # ### dat.save_pkl( test_class, path + 'test_class_30000' )
    # # about train set
    #     tra_class=classify_multi(tra_seg,300,center)
    #     dat.save_pkl(tra_class,path+'tra_class_'+str(w))
    ###########################################################
    ## extract trajectories with completion
    tra_p_all= dat.load_pkl( path + 'test_p' )#train
    tra_p={}
    for k in tra_p_all:
        tra_p[k]=tra_p_all[k]
    center = dat.load_pkl( path + 'res_clu_train_30000' )
    completion( tra_p, center, 'train' )
    ...
    # # train(tra_class,center,lab_true,layer=1)
    # # # # # ### train the model when the completion is less than 100%
    # com=Com
    # tra_class_c={}
    # for k in tra_class:
    #     tra_class_c[k]=tra_class[k][:int(tra_class[k].__len__()*(com/10)+0.5)]
    #     # print(tra_class[k][:int(tra_class[k].__len__()*(com/10)+0.5)])
    ...
    # using=layer
    # for u in using:
    #     # name_model=path+'model_batch/'+'1_128_'+str(u) # batch
    #     # name_model=path+'model_layer/'+str(u)+'_128_64'# layer
    #     name_model=path+'model_neuron/'+'1_'+str(u)+'_64' # neuron
    ...
    #     print(name_model,'mean error:', error, 'km' )
